...
def beam(model, data_iter, args):
    ...
    with torch.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            history = None
            with torch.no_grad():
                # print("args.eval_len:", args.eval_len)
                for i in range(0, args.eval_len):
                    if dist.get_rank() > 0:
                        if i == 0:
                            logits, past_lora, lora_info = model(_query)
                            logits = logits[_bbatch, (_query_len-1).long(), :] # batch_size * beam, vocab
                        else:
                            dist.send(len_past, dst=0)
                            logits, past_lora, lora_info = model(token_id, len_past=len_past)  #
                            logits = logits[:, -1, :]    # batch_size * beam, vocab
                    else:
                        if i == 0:
                            logits_base, past, base_info = model(_query.size(), lm_labels=args.device)
                        else:
                            len_past_from_lora = torch.empty(
                                (num_beams), dtype=torch.long).to(args.device)
                            dist.recv(tensor=len_past_from_lora, src=1)
                            len_past = len_past_from_lora
                            logits_base, past, base_info = model(torch.Size((num_beams, batch_size)), lm_labels=args.device, past=past, len_past=len_past)  
                    ...
                    if dist.get_rank() > 0:
                        ...
                        dist.send(beam_idx, dst=0)
                    if dist.get_rank() == 0:
                        beam_id_from_lora = torch.empty(
                            (batch_size, num_beams), dtype=torch.long).to(args.device)
                        dist.recv(tensor=beam_id_from_lora, src=1)
                        beam_idx = beam_id_from_lora
...
if __name__ == '__main__':
    ...
    config.optimize_tool = {}
    config.optimize_tool['head_alloc_device'] = [8,5,3]
