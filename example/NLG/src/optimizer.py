...
def create_adam_optimizer_from_args(model, args, grouped_parameters=None):
    ...
    # print("grouped_parameters:", type(grouped_parameters[0]))
    # grouped_parameters = filter(lambda p: p.requires_grad, model.parameters())
    optimizer = AdamW(
        grouped_parameters, 
        lr=args.lr, 
    ...
    #     print(f"Shape: {param.size()}, Device: {param.device}")
    return optimizer
